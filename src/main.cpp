...
void LoadingLayer_loadAssets(gd::LoadingLayer* self)
{
   ...
   // pauseBtn->setPosition({winSize.width * 0.15f, winSize.height - 25});
	...
	for(unsigned int i = 0; i < self->getChildrenCount(); i++)
	{
		...
		if(auto s = dynamic_cast<CCSprite*>(o)) 
		{
			...
			else if(s->isVisible() && (strcmp(tname, "GJ_featuredCoin_001.png") == 0)) 
			{
				num = 3;
				s->setTag(120);
			}
...
void LevelInfoLayer_updateLabelValues(gd::LevelInfoLayer* self)
{
...
void OptionsLayer_customSetup(gd::OptionsLayer* self)
{
...
void LevelCell_loadCustomLevelCell(gd::LevelCell* self) {
...
void ProfilePage_loadPageWithUserInfo(gd::ProfilePage *self, gd::GJUserScore *score)
{
...
void OptionsLayer_musicSliderChanged(cocos2d::CCObject *a1)
{
